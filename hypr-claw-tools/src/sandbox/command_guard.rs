...
impl CommandGuard {
    pub fn validate(cmd: &[String]) -> Result<(), ToolError> {
        ...
        if BLACKLIST.iter().any(|&b| program.contains(b)) {
            return Err(ToolError::SandboxViolation(format!("Blocked command: {}", program)));
        }
        ...
        if !WHITELIST.contains(&base_cmd) {
            return Err(ToolError::SandboxViolation(format!("Command not whitelisted: {}", base_cmd)));
        }
    ...
    fn validate_argument(arg: &str, is_program: bool) -> Result<(), ToolError> {
        ...
        if arg.chars().any(|c| c.is_control() && c != '\t') {
            return Err(ToolError::SandboxViolation("Control character in argument".into()));
        }
        ...
        if arg.contains("..") {
            return Err(ToolError::SandboxViolation("Path traversal in argument".into()));
        }
        ...
        for &sensitive in SENSITIVE_PATHS {
            if arg.starts_with(sensitive) {
                return Err(ToolError::SandboxViolation(
                    format!("Access to sensitive path: {}", sensitive)
                ));
            }
        }
        ...
        if arg.starts_with('/') && !arg.starts_with("/tmp") {
            return Err(ToolError::SandboxViolation("Absolute path not allowed".into()));
        }

        // Check for git config manipulation
        if arg.starts_with("--global") || arg.starts_with("--system") {
            return Err(ToolError::SandboxViolation("Global/system config not allowed".into()));
        }

        // Check for -C flag (change directory)
        if arg == "-C" {
            return Err(ToolError::SandboxViolation("Directory change not allowed".into()));
        }
    ...
    fn validate_git_command(args: &[String]) -> Result<(), ToolError> {
        ...
        if !GIT_ALLOWED_SUBCOMMANDS.iter().any(|&allowed| subcommand == allowed) {
            return Err(ToolError::SandboxViolation(
