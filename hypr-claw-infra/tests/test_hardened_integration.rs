...
fn test_full_hardened_workflow() {
    ...
    let audit_logger = audit_logger_chained::AuditLogger::new(dir.path().join("audit.log")).unwrap();
...
fn test_hardened_concurrent_workflow() {
    ...
    let session_store = Arc::new(session_store::SessionStore::new(dir.path().join("sessions")).unwrap());
    let audit_logger = Arc::new(audit_logger_chained::AuditLogger::new(dir.path().join("audit.log")).unwrap());
...
fn test_hardened_integrity_validation() {
    ...
    session_store.append("test", &serde_json::json!({"test": true})).unwrap();
...
fn test_hardened_crypto_security() {
    ...
    let store2 = credential_store::CredentialStore::new(dir.path().join("creds"), &wrong_key).unwrap();
