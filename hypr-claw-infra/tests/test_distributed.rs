...
fn test_distributed_lock_trait_conformance() {
    ...
    let lock = manager.acquire("test_session", Duration::from_secs(1)).unwrap();
...
fn test_mock_network_failure() {
    ...
    impl DistributedLockManager for FailingLockManager {
        fn acquire(&self, _session_key: &str, _timeout: Duration) -> Result<Box<dyn DistributedLock>, DistributedError> {
            Err(DistributedError::Network("Connection refused".to_string()))
        }
        
        fn try_acquire(&self, _session_key: &str) -> Result<Option<Box<dyn DistributedLock>>, DistributedError> {
            Err(DistributedError::Network("Connection refused".to_string()))
        }
...
fn test_failover_behavior() {
    ...
    let lock = local_manager.acquire("session1", Duration::from_secs(1)).unwrap();
