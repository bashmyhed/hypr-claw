...
impl AuditLogger {
    ...
    fn verify_and_get_last_hash(log_path: &Path) -> Result<String, AuditLoggerError> {
        ...
        for line in reader.lines() {
            ...
            let chained_entry: ChainedAuditEntry = serde_json::from_str(&line)
                .map_err(|e| AuditLoggerError::IntegrityViolation(
                    format!("Line {}: Invalid JSON: {}", line_num, e)
                ))?;
            
            // Verify prev_hash matches
            if chained_entry.prev_hash != prev_hash {
                return Err(AuditLoggerError::IntegrityViolation(
                    format!("Line {}: Hash chain broken. Expected prev_hash '{}', got '{}'",
            ...
            if computed_hash != chained_entry.entry_hash {
                return Err(AuditLoggerError::IntegrityViolation(
                    format!("Line {}: Hash mismatch. Expected '{}', got '{}'",
