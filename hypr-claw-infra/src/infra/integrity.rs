...
impl IntegrityValidator {
    ...
    fn verify_session_store(store_path: &Path) -> Result<(), IntegrityError> {
        ...
        for entry in entries {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("jsonl") {
                let session_key = path.file_stem()
                    .and_then(|s| s.to_str())
                    .ok_or_else(|| IntegrityError::SessionData("Invalid filename".to_string()))?;
                
                // Try to load session
                store.load(session_key)
                    .map_err(|e| IntegrityError::SessionData(
                        format!("Session '{}': {}", session_key, e)
                    ))?;
            }
        }
    ...
    fn verify_database(db_path: &Path) -> Result<(), IntegrityError> {
        ...
        if result != "ok" {
            return Err(IntegrityError::Database(format!("Integrity check failed: {}", result)));
        }
...
mod tests {
    ...
    fn test_corrupted_session_json() {
        ...
        fs::write(&session_file, "{\"valid\": true}\n{invalid}\n{\"also_valid\": true}\n").unwrap();
