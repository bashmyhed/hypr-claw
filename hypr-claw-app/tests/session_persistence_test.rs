...
async fn test_session_persistence_across_restarts() {
    ...
    // First execution - create and save session manually
    {
        let session_store = Arc::new(
            hypr_claw::infra::session_store::SessionStore::new(test_dir).unwrap()
        );
    ...
    // Simulate restart - create new runtime with same session store
    {
        ...
        assert!(!messages.is_empty(), "Session should contain messages after restart");
...
async fn test_session_not_corrupted_on_error() {
    ...
    
    {
        ...
        let lock_manager = Arc::new(
            hypr_claw::infra::lock_manager::LockManager::new(Duration::from_secs(30))
        );
